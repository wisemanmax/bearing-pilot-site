 ...
 *
 * On a touch device the two inputs are replaced and nothing else changes.
 * `opts.touch` swaps pointer lock for an explicit `active` flag (there is no
 * lock to take, and no Esc to release it), and `opts.input` supplies the
 * three axes the keyboard used to: forward, sideways, and whether this is a
 * run.  The look deltas arrive the same way the mouse's did -- as pixels,
 * drained once per frame -- so the sensitivity, the clamp and the bank all
 * work on touch without being told about it.
 * ------------------------------------------------------------------ */
...
export class Player {
  constructor(camera, domElement, world, opts = {}) {
    ...
    /** Touch mode: no pointer lock, and movement comes from `input`. */
  ...
  _bind() {
    /* Pointer lock is bound only when there is one to take.  Binding it on a
     * phone is not merely useless: `pointerlockchange` never fires, so
     * `locked` would stay false forever and the walker would never move. */
    if (!this.touch) {
      const onMove = (e) => {
    ...
    /* The keyboard stays bound in touch mode.  An iPad with a Magic Keyboard
     * is a touch device with WASD on it, and there is no reason to refuse. */
  ...
  /** Touch mode only: the desktop path releases through `pointerlockchange`. */
  unlock() {
  ...
  update(dt) {
    ...
    /* Look first, so the movement below is resolved against the heading the
     * frame is actually going to be drawn with.  Touch look arrives in pixels
     * accumulated since the last frame, which is the same quantity
     * `movementX` delivers -- so it goes through the same sensitivity and the
     * same pitch clamp, and the bank in `applyCamera` reads it unchanged. */
    if (this.locked && this.input) {
      const look = this.input.takeLook();
      if (look.dx || look.dy) {
        const s = this.input.sensitivity ?? this.sensitivity;
        this.yaw -= look.dx * s;
        this.pitch -= look.dy * s;
        this.pitch = clamp(this.pitch, -1.15, 1.05);
      }
    }
    ...
    if (this.locked) {
      ...
      /* The stick is analogue and the keyboard is not, so whichever is
       * pushed further wins rather than the two being summed -- otherwise a
       * half-deflected stick and a held W make 1.5 and the walker outruns
       * its own animation. */
      if (this.input) {
        if (Math.abs(this.input.fwd) > Math.abs(fwd)) fwd = this.input.fwd;
        if (Math.abs(this.input.side) > Math.abs(side)) side = this.input.side;
        sprint = sprint || this.input.run;
      }
    }
    ...
    /* The stick is analogue where the keyboard was not, so the wish speed is
     * scaled by how far it is pushed.  With a keyboard `fwd` and `side` are
     * exactly 0 or ±1, so this is a no-op there and the walk is unchanged. */
    if (riding) {
      // no lateral component at all: the machine only goes where it points
      this._wish.copy(this._forward)
        .multiplyScalar(fwd > 0.02 ? speed * Math.min(1, fwd) : fwd < -0.02 ? -RIDE.reverse : 0);
    } else {
      this._wish
        .copy(this._forward).multiplyScalar(fwd)
        .addScaledVector(this._right, side);
      const mag = Math.min(1, Math.hypot(fwd, side));
      if (this._wish.lengthSq() > 1e-6) this._wish.normalize().multiplyScalar(speed * mag);
    }
  ...
  /**
   * Ray-test from an arbitrary screen point -- what a tap does.
   *
   * Slightly longer reach than the crosshair pick, and deliberately: aiming
   * a crosshair is a continuous correction and you stop when it lights up,
   * whereas a tap is committed before you know whether it landed.  A little
   * extra range is what makes a tap that was *nearly* right still work.
   *
   * @param ndc  the tap in normalised device coordinates
   */
  pickAt(ndc, interactables, reach = 4.2) {
    ...
    return hits.length ? interactables[meshes.indexOf(hits[0].object)] : null;
  }
}
